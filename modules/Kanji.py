...
def get_kanji(level, start, end):
    while(start <= end):
        ...
        url = (f'https://jisho.org/search/%23kanji%20%23jlpt-n{level}?page={start}')  # noqa: E501
        ...
        for kanji in get_kanji:
            character = kanji.find('span',
                                   class_='character literal japanese_gothic')\
                .text.replace('\n', '').strip()
            meaning = kanji.find('div', class_='meanings english sense')\
                .text.replace('\n', '').strip()
            get_kun_reading = kanji.find('div', class_='kun readings')
            if get_kun_reading is None:
                kun_reading = ''
            else:
                kun_reading = kanji.find('div', class_='kun readings')\
                    .text.replace('\n', '').strip()
            get_on_reading = kanji.find('div', class_='on readings')
            if get_on_reading is None:
                on_reading = ''
            else:
                on_reading = kanji.find('div', class_='on readings')\
                    .text.replace('\n', '').strip()
